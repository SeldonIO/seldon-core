...
If external access to the elasticsearch backend is needed then the chart supports ingress or you can run `kubectl patch svc elasticsearch-master -n logs -p '{"spec": {"type": "LoadBalancer"}}'`
