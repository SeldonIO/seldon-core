from seldon_core.microservice_tester import run_predict, run_send_feedback
...
def test_predict_rest(mock_post):
    ...
    args_dict = {"contract":filename,"host":"a","port":1000,"n_requests":1,"batch_size":1,"endpoint":"predict","prnt":True,"grpc":False,"tensor":True}
...
def test_feedback_rest(mock_post):
    filename = join(dirname(__file__), "model-template-app", "contract.json")
    args_dict = {"contract":filename,"host":"a","port":1000,"n_requests":1,"batch_size":1,"endpoint":"feedback","prnt":True,"grpc":False,"tensor":True}
    args = Bunch(args_dict)
