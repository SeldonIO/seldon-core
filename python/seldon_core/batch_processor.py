    ...
    try:
        if data_type == "data":
            ...
            for i, raw_data in enumerate(input_raw):
                ...
                data_np = np.array(data, dtype="O").reshape(1, -1)

                ids.append((data_np[0, 0]))
                overall = np.concatenate((overall, data_np[0, 1:].reshape(1, -1)))
...
def _send_batch_predict(
    ...
    try:
        ...
        if data_type == "data":
            data_np = np.array(data, dtype="O")
            # need the data be the id
            if (len(data_np) == 1):  ###in this case, it is only possible to be id
                meta['tags']['id'] = (data_np[0])
