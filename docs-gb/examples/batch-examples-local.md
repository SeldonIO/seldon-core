...
customized this, you'll need to [redirect the CLI](../cli/).
{% endhint %}
...
check that they work with a single standalone inference request. We can do this using the`seldon model infer` command.
...
to a file `/tmp/iris-output.txt`. For a full set of options check out the[MLServer CLI Reference](https://mlserver.readthedocs.io/en/latest/reference/cli.html#mlserver-infer).
