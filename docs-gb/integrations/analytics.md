...
* deployment\_name
* predictor\_name
* predictor\_version (This will be derived from the predictor metadata labels)
* model\_name
* model\_image
* model\_version
...
![prometheus-targets](../.gitbook/assets/prometheus-targets.png)
...
in the Prometheus [config](../../helm-charts/seldon-core-analytics/files/prometheus/prometheus-config.yaml) together with following two annotations:
...
Before Seldon Core 1.1 custom metrics have been returned to the orchestrator which exposed them all together to `Prometheus` via a single endpoint. We used to have at this time all three following annotations:
