...
![mlflow-dashboard](../.gitbook/assets/mlflow-dashboard.png)
...
We will be using the model we updated to our google bucket (gs://seldon-models/mlflow/elasticnet\_wine\_1.8.0), but you can use your model if you uploaded it to a public bucket.
...
, where $R(x\_{n})$ is the reward for input point $x\_{n}$, $f(x\_{n})$ is our trained model and $y\_{n}$ is the actual value.
...
![grafana-mlflow](../.gitbook/assets/grafana-mlflow.jpg)
...
You are able to add your own custom metrics, and try out other more complex deployments by following [these guides](./)
