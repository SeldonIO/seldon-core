...
def update_kustomize_engine_version(seldon_core_version, debug=False):
    args = ["sed", "-i", "s/docker.io\/seldonio\/engine:\(.*\)/docker.io\/seldonio\/engine:{seldon_core_version}/g".format(**locals()),"operator/config/manager/manager.yaml"]
    err, out = run_command(args, debug)
    #pp(out)
    #pp(err)
    if err == None:
...
def main(argv):
    ...
    CHART_YAML_FILES = ['helm-charts/seldon-core-operator/Chart.yaml', 'helm-charts/seldon-core-analytics/Chart.yaml']
    OPERATOR_VALUES_YAML_FILE = 'helm-charts/seldon-core-operator/values.yaml'

    opts = getOpts(argv[1:])
    if opts.debug:
        pp(opts)
    set_version(opts.seldon_core_version, POM_FILES, CHART_YAML_FILES, OPERATOR_VALUES_YAML_FILE, opts.debug)
    print("done")
