...
oidc_resource = os.getenv("OIDC_RESOURCE")
...
default_mapping = {
    "properties": {
        "@timestamp": {"type": "date"},
        "Ce-Endpoint": {"type": "keyword"},
        "Ce-Inferenceservicename": {"type": "keyword"},
        "Ce-Modelid": {"type": "keyword"},
        "Ce-Namespace": {"type": "keyword"},
        "RequestId": {"type": "keyword"},
        "ServingEngine": {"type": "keyword"},
        "request": {
            "properties": {
                "ce-source": {"type": "keyword", "index": "false"},
                "ce-time": {"type": "date", "index": "false"},
    ...
    if not index_exist:
        ...
        try:
            ...
            elastic_object.indices.create(
                index=index_name, body={"mappings": mapping_body}
            )
...
def get_index_mapping(index_name, upsert_body):
    ...
    inferenceservice_name = (
        upsert_body[log_helper.INFERENCESERVICE_HEADER_NAME]
        if log_helper.INFERENCESERVICE_HEADER_NAME in upsert_body
        else ""
    )
    namespace_name = (
        upsert_body[log_helper.NAMESPACE_HEADER_NAME]
        if log_helper.NAMESPACE_HEADER_NAME in upsert_body
        else ""
    )
    endpoint_name = (
        upsert_body[log_helper.ENDPOINT_HEADER_NAME]
        if log_helper.ENDPOINT_HEADER_NAME in upsert_body
        else ""
    )
    serving_engine = (
        upsert_body["ServingEngine"] if "ServingEngine" in upsert_body else "seldon"
    )
    ...
    if not metadata or metadata is None:
        ...
        if "requests" in metadata:
            ...
            if req_mapping != None:
                index_mapping["properties"]["request"]["properties"][
                    "elements"
                ] = req_mapping
        ...
        if "responses" in metadata:
            ...
            if resp_mapping != None:
                index_mapping["properties"]["response"]["properties"][
                    "elements"
                ] = resp_mapping
        return index_mapping
...
def fetch_metadata(namespace, serving_engine, inferenceservice_name, predictor_name):
    ...
    if not deployment_type:
        print(
            "unknown deployment type for " + namespace + " / " + inferenceservice_name
        )
    ...
    runtime_metadata = (
        metadata_api.model_metadata_service_list_runtime_metadata_for_model(
            deployment_name=inferenceservice_name,
    ...
    if (
        ...
        and runtime_metadata.runtime_metadata is not None
        and runtime_metadata.runtime_metadata
    ):
        ...
        model_metadata = metadata_api.model_metadata_service_list_model_metadata(
            uri=model_uri
        )
        if model_metadata is None or len(model_metadata.models) == 0:
            print(
                "no model corresponding to runtime metadata "
                + namespace
                + "/"
                + inferenceservice_name
            )
...
def update_props_element(props, elm):
    if not ("type" in elm):
        props[elm["name"]] = {"type": "float"}  # TODO: Use data type if available
        return props

    else:
        if elm["type"] == "CATEGORICAL":
            props[elm["name"]] = {"type": "keyword"}
            return props
        if elm["type"] == "TEXT":
            props[elm["name"]] = {"type": "text"}
            return props
