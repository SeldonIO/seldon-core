    ...
    def process_event(self, inputs: Union[List, Dict], headers: Dict) -> Dict:
        ...
        if "response" in inputs:
            ...
            if self.name == "BINARY_CLASSIFICATION":
                response = int(response) if isinstance(response, list) else int(response[0])
                truth = int(truth) if isinstance(truth, list) else int(truth[0])
            ...
            elif self.name == "MULTICLASS_CLASSIFICATION_ONE_HOT":
                pass
