...
func TestKafkaConfig_GetNumPartitions(t *testing.T) {
	g := NewGomegaWithT(t)

	type test struct {
		name     string
		data     kafka.ConfigMap
		expected int
		err      bool
	}
	...
	tests := []test{
		{
			name:     "success: integer in numPartitions",
			data:     kafka.ConfigMap{"replication.factor": 1, "numPartitions": 1},
			expected: 1,
			err:      false,
		},
		{
			name:     "fail: negative numPartitions",
			data:     kafka.ConfigMap{"replication.factor": 1, "numPartitions": -10},
			expected: -1,
			err:      true,
		},
		{
			name:     "fail: invalid int input",
			data:     kafka.ConfigMap{"replication.factor": 1, "numPartitions": 0},
			expected: -1,
			err:      true,
		},
		{
			name:     "fail: float int numPartitions",
			data:     kafka.ConfigMap{"replication.factor": 1, "numPartitions": 0.0},
			expected: -1,
			err:      true,
		},
		{
			name:     "success: string in numPartitions",
			data:     kafka.ConfigMap{"replication.factor": 1, "numPartitions": "1"},
			expected: 1,
			err:      false,
		},
		...
		{
			name:     "fail: string and negative numPartitions",
			data:     kafka.ConfigMap{"replication.factor": 1, "numPartitions": "-10"},
			expected: -1,
			err:      true,
		},
		{
			name:     "fail: invalid string input",
			data:     kafka.ConfigMap{"replication.factor": 1, "numPartitions": "-10-"},
			expected: -1,
			err:      true,
		},
		...
		{
			name:     "fail: float string numPartitions",
			data:     kafka.ConfigMap{"replication.factor": 1, "numPartitions": "0.0"},
			expected: 0,
			err:      true,
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			kc := KafkaConfig{}
			kc.Topics = test.data
			numPartitions, err := kc.GetNumPartitions()
			if test.err {
				g.Expect(err).ToNot(BeNil())
			} else {
				g.Expect(numPartitions).To(Equal(test.expected))
			}
		})
	}
}
