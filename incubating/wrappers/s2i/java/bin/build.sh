...
if [[ -n "${S2I_DOCKER_PLATFORM:-}" ]]; then
  IMAGE_PLATFORM_ARGS+=(--platform="${S2I_DOCKER_PLATFORM}")
elif [[ "$(uname -m)" != "x86_64" ]]; then
  IMAGE_PLATFORM_ARGS+=(--platform=linux/amd64)
fi

# Prepare Docker build args ( if supplied )
if [[ -n "$BUILD_SOURCE_IMAGE" ]]; then
  IMAGE_BUILD_BUILD_ARGS+=(--build-arg "IMAGE_SOURCE=$BUILD_SOURCE_IMAGE")
fi
if [[ -n "$RUNTIME_SOURCE_IMAGE" ]]; then
  IMAGE_RUNTIME_BUILD_ARGS+=(--build-arg "IMAGE_SOURCE=$RUNTIME_SOURCE_IMAGE")
fi
