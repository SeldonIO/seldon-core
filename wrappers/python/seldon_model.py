...
def get_grpc_features(datadef):
    if len(datadef.tensor.values)>0:
        return np.array(datadef.tensor.values).reshape(datadef.tensor.shape),datadef.features
    else:
        return np.array(datadef.ndarray),datadef.features
...
def get_rest_features(data):
    ...
    return values, data.get("features")
