...
<a id="v2.9.0"></a>
## [v2.9.0](https://github.com/SeldonIO/seldon-core/releases/tag/v2.9.0) - 2025-04-07
...
Core 2 also has new [docs](https://docs.seldon.ai/seldon-core-2), with revamped content and structure. Documentation will continuously improve to address advanced configurations and use cases.  

### CRD Updates:
All CRD changes in this release maintain backward compatibility, so clusters with existing CRs can be migrated seamlessly.

* Add `status.availableReplicas` field to the Model CRD ([#5873](https://github.com/SeldonIO/seldon-core/issues/5873)). Part of the partial scheduling feature. Field not directly set by end-users, but updated by the seldon k8s operator
* Add `spec.llm` field to the Model CRD ([#6234](https://github.com/SeldonIO/seldon-core/issues/6234)). The field is used by the PromptRuntime (in Seldon's LLM Module) to reference a LLM model. Only one of `spec.llm` and `spec.explainer` should be set at a given time. This allows the deployment of multiple "models" acting as prompt generators for the same LLM.

### (Main) Features:

* We add **model response streaming** support for REST (via [SSE](https://html.spec.whatwg.org/multipage/server-sent-events.html)) and gRPC for MLServer models that have streaming support ([#6293](https://github.com/SeldonIO/seldon-core/issues/6293), [#6292](https://github.com/SeldonIO/seldon-core/issues/6292)). This requires MLServer `>= 1.6.0`.

* We introduce **partial scheduling for model replicas** ([#6221](https://github.com/SeldonIO/seldon-core/issues/6221), [docs](https://github.com/SeldonIO/seldon-core/blob/release-2.9/docs-gb/models/scheduling.md)), improving the behaviour of Core 2 during autoscaling. With this new feature, the Core 2 scheduler will try to load as many of the requested model replicas as possible, even when no inference server has sufficient replicas to meet this request. 
	**Partial scheduling is only active when end-users provide `spec.minReplicas` in a model manifest** (as a user-provided minimum for considering the model "available"), and takes effect when there is a suitable inference server with at least this number of replicas. With partial scheduling, a model can be:
	...
	- Not scheduled: no suitable inference servers that have a number of replicas greater or equal to the model's `spec.minReplicas` were found. The `ModelReady` condition is `False` with message `ScheduleFailed`. The model is unavailable for inference requests.

* We introduce **mixed native/HPA autoscaling** ([#6218](https://github.com/SeldonIO/seldon-core/issues/6218), [#6222](https://github.com/SeldonIO/seldon-core/issues/6222), [#6235](https://github.com/SeldonIO/seldon-core/issues/6235), with docs for [model](https://docs.seldon.ai/seldon-core-2/user-guide/scaling/autoscaling) and [server](https://docs.seldon.ai/seldon-core-2/user-guide/scaling/server-autoscaling) autoscaling) that:
	...
	We also introduce **experimental functionality to pack models** on fewer inference servers on model scale-down, but this is disabled by default and will be improved in future releases. See the [scale-down docs](https://docs.seldon.ai/seldon-core-2/user-guide/scaling/server-autoscaling#policies-1) for details.

* Model scheduling now **takes into account** model memory requirements based on the inference server config and **how many in-memory copies of one model it creates** (the `parallel_workes` MLServer setting and `instance_group` configurations in Triton). For triton, only `KIND_CPU` instance groups are considered at this point ([#6054](https://github.com/SeldonIO/seldon-core/issues/6054))

* Log levels for all internal components ([#6312](https://github.com/SeldonIO/seldon-core/issues/6312)) and the envoy accesslog ([#6295](https://github.com/SeldonIO/seldon-core/issues/6295)) can now be controlled in a consistent way.

#### Features configuration & helm chart updates

* Server `spec.minReplicas` and `spec.maxReplicas` can be configured via helm ([#6283](https://github.com/SeldonIO/seldon-core/issues/6283)) via the following values: 
	- `mlserver.minReplicas` 
	- `mlserver.maxReplicas` 
	- `triton.minReplicas` 
	- `triton.maxReplicas` 

* Native autoscaling features control ([#6301](https://github.com/SeldonIO/seldon-core/issues/6301), [#6286](https://github.com/SeldonIO/seldon-core/issues/6286)). All options here have corresponding command-line arguments that can be passed to `seldon-scheduler` when not using helm as the install method. The following helm values can be set
...
* The inference server [PVC retention policy](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/#persistentvolumeclaim-retention) can now be configured via helm  ([#6056](https://github.com/SeldonIO/seldon-core/issues/6056)) via the following values: 
...
* Logging options can be configured either via helm values or by passing arguments to individual Core 2 components. Docs for [component log levels](https://docs.seldon.ai/seldon-core-2/installation/advanced-configurations/helm#component-log-level) and the [envoy access log](https://docs.seldon.ai/seldon-core-2/installation/advanced-configurations/helm#access-log) are available 
	
Please consult the [helm chart docs](https://docs.seldon.ai/seldon-core-2/installation/advanced-configurations/helm) for a full list of options


### (Main) Bugs Fixed:

* Transient NC 503 error in envoy logs when rolling out a new version of a model
	- fixed in [#6082](https://github.com/SeldonIO/seldon-core/issues/6082) by configuring envoy to use the Aggregated Discovery Service (ADS) in order to add guarantees regarding the order of routing updates coming from the Core 2 scheduler.
* Mislabelled operational metrics for experiments in Prometheus
	- fixed in [#6118](https://github.com/SeldonIO/seldon-core/issues/6118) by recording the actual model name in the `model` label rather than the experiment name;
* Some errors encountered by `modelgateway` were silently ignored
	- fixed in [#6014](https://github.com/SeldonIO/seldon-core/issues/6014) by propagating those errors via Kafka, by writing into an error topic (DLQ)
* `seldon-scheduler` pod spec overrides (alongisde any other stateful-set pod spec overrides) defined within the SeldonRuntime CR were ignored
	- fixed in [#6349](https://github.com/SeldonIO/seldon-core/issues/6349)

#### Priority bugfixes scheduled for the _next_ patch releases of Core 2.9
...
* Sherif Akoush ([@sakoush](https://github.com/sakoush))

------

### Changelog

Dates are displayed in UTC. Generated by [`auto-changelog`](https://github.com/CookPete/auto-changelog).

#### [v2.9.0](https://github.com/SeldonIO/seldon-core/compare/v2.8.5...v2.9.0)

> 7 April 2025

- fix(dataflow): Update default kafka log level for dataflow engine [`#6367`](https://github.com/SeldonIO/seldon-core/pull/6367)
- feat(docs): Server native autoscaling [`#6356`](https://github.com/SeldonIO/seldon-core/pull/6356)
- Bump ubi9/openjdk-17-runtime from 1.20 to 1.22 in /scheduler [`#6359`](https://github.com/SeldonIO/seldon-core/pull/6359)
- docs(pipelines): Minor Pipelines doc improvements [`#6351`](https://github.com/SeldonIO/seldon-core/pull/6351)
- fix: adjust logging level for dataflow [`#6350`](https://github.com/SeldonIO/seldon-core/pull/6350)
- fix(operator): Apply scheduler runtime podSpec override [`#6349`](https://github.com/SeldonIO/seldon-core/pull/6349)
- Spelling fix [`#6340`](https://github.com/SeldonIO/seldon-core/pull/6340)
- Add a missing space [`#6338`](https://github.com/SeldonIO/seldon-core/pull/6338)
- Re-generate license info [`#6333`](https://github.com/SeldonIO/seldon-core/pull/6333)
- Re-generate license info [`#6332`](https://github.com/SeldonIO/seldon-core/pull/6332)
- fix (docs) incorporate mark's suggestions [`#6313`](https://github.com/SeldonIO/seldon-core/pull/6313)
- fix(scheduler): Do not try to unload versions that are not live [`#6331`](https://github.com/SeldonIO/seldon-core/pull/6331)
- (re)allow triton rclone port to be set in compose [`#6330`](https://github.com/SeldonIO/seldon-core/pull/6330)
- fix(gha): update upload-artifact version [`#6329`](https://github.com/SeldonIO/seldon-core/pull/6329)
- feat: Config logging via helm [`#6312`](https://github.com/SeldonIO/seldon-core/pull/6312)
- fix: Allow kafka client (`librdkafka`)  to respect log level [`#6310`](https://github.com/SeldonIO/seldon-core/pull/6310)
- docs: fix rewrite of About [`#6296`](https://github.com/SeldonIO/seldon-core/pull/6296)
- Bump sigs.k8s.io/controller-runtime from 0.20.1 to 0.20.3 in /scheduler [`#6307`](https://github.com/SeldonIO/seldon-core/pull/6307)
- Bump grafana/grafana from 11.5.1 to 11.5.2 in /scheduler [`#6289`](https://github.com/SeldonIO/seldon-core/pull/6289)
- feat: add helm for disabling native autoscaling feature [`#6301`](https://github.com/SeldonIO/seldon-core/pull/6301)
- feat(docs): add docs for access log and autoscaling helm config [`#6300`](https://github.com/SeldonIO/seldon-core/pull/6300)
- Update SUMMARY.md [`#6303`](https://github.com/SeldonIO/seldon-core/pull/6303)
- feat: implemented grpc model streaming [`#6293`](https://github.com/SeldonIO/seldon-core/pull/6293)
- IA  mapping [`#6236`](https://github.com/SeldonIO/seldon-core/pull/6236)
- feat(envoy): enable accesslog configuration [`#6295`](https://github.com/SeldonIO/seldon-core/pull/6295)
- [update] Operational Monitoring new IA [`#6257`](https://github.com/SeldonIO/seldon-core/pull/6257)
- docs fix for API [`#6294`](https://github.com/SeldonIO/seldon-core/pull/6294)
- test(scheduler): Included infer_stream test for REST [`#6292`](https://github.com/SeldonIO/seldon-core/pull/6292)
- feat: Enable packing configuration via helm [`#6286`](https://github.com/SeldonIO/seldon-core/pull/6286)
- fix(docs) istio.md [`#6287`](https://github.com/SeldonIO/seldon-core/pull/6287)
- fix(operator): allow scaling requests for older generation [`#6285`](https://github.com/SeldonIO/seldon-core/pull/6285)
- fix(scheduler): Remove all versions when model is deleted [`#6284`](https://github.com/SeldonIO/seldon-core/pull/6284)
- Bump rclone/rclone from 1.69.0 to 1.69.1 in /scheduler [`#6272`](https://github.com/SeldonIO/seldon-core/pull/6272)
- adding min and max replicas to the server helm chart [`#6283`](https://github.com/SeldonIO/seldon-core/pull/6283)
- fix(scheduler): ignoring model runtime info in model equality check [`#6259`](https://github.com/SeldonIO/seldon-core/pull/6259)
- feat(scheduler): Scale down server logic [`#6246`](https://github.com/SeldonIO/seldon-core/pull/6246)
- bug(scheduler): do not scale to zero if max replicas is missing [`#6258`](https://github.com/SeldonIO/seldon-core/pull/6258)
- feat(scheduler): mms send scaling request when model shceduling fails [`#6235`](https://github.com/SeldonIO/seldon-core/pull/6235)
- Re-generate license info [`#6256`](https://github.com/SeldonIO/seldon-core/pull/6256)
- [update] installation draft [`#6131`](https://github.com/SeldonIO/seldon-core/pull/6131)
- feat(scheduler): Allow server stats to be returned [`#6253`](https://github.com/SeldonIO/seldon-core/pull/6253)
- Bump github.com/envoyproxy/go-control-plane/envoy in /scheduler [`#6249`](https://github.com/SeldonIO/seldon-core/pull/6249)
- Bump google.golang.org/protobuf from 1.36.4 to 1.36.5 in /operator [`#6250`](https://github.com/SeldonIO/seldon-core/pull/6250)
- upgrade lint to v1.63.4 [`#6247`](https://github.com/SeldonIO/seldon-core/pull/6247)
- fix(ci): builld k6 image - pin xk6 version to 0.13.4 [`#6245`](https://github.com/SeldonIO/seldon-core/pull/6245)
- use go 1.22 for k6 image build [`#6244`](https://github.com/SeldonIO/seldon-core/pull/6244)
- Re-generate license info [`#6243`](https://github.com/SeldonIO/seldon-core/pull/6243)
- fix: Upgrade Go 1.23 and dependencies upgrade [`#6238`](https://github.com/SeldonIO/seldon-core/pull/6238)
- Re-generate license info [`#6242`](https://github.com/SeldonIO/seldon-core/pull/6242)
- fix(docs):  Document Scheduling logic [`#6237`](https://github.com/SeldonIO/seldon-core/pull/6237)
- Bump grafana/grafana from 11.4.0 to 11.5.1 in /scheduler [`#6240`](https://github.com/SeldonIO/seldon-core/pull/6240)
- Bump rclone/rclone from 1.68.2 to 1.69.0 in /scheduler [`#6192`](https://github.com/SeldonIO/seldon-core/pull/6192)
- fix(operator): Add Status.AvailableReplicas to Model CRD [`#5873`](https://github.com/SeldonIO/seldon-core/pull/5873)
- feat(operator): Included LLM spec to CRD [`#6234`](https://github.com/SeldonIO/seldon-core/pull/6234)
- feat(scheduler): add partial scheduling based on min replicas [`#6221`](https://github.com/SeldonIO/seldon-core/pull/6221)
- feat(operator): adding a patch for server/spec/replicas upon scaling request [`#6222`](https://github.com/SeldonIO/seldon-core/pull/6222)
- fix for envoy configs [`#6220`](https://github.com/SeldonIO/seldon-core/pull/6220)
- Bump envoyproxy/envoy from v1.32.2 to v1.33.0 in /scheduler [`#6206`](https://github.com/SeldonIO/seldon-core/pull/6206)
- feat: enable min/max replica for Server CR [`#6218`](https://github.com/SeldonIO/seldon-core/pull/6218)
- new introduction to Core 2 [`#6195`](https://github.com/SeldonIO/seldon-core/pull/6195)
- feat(envoy): use the healthcheck filter and a prestop hook to gracefully terminate Envoy [`#6194`](https://github.com/SeldonIO/seldon-core/pull/6194)
- Re-generate license info [`#6184`](https://github.com/SeldonIO/seldon-core/pull/6184)
- feat(scheduler): account for number of model instances when scheduling [`#6183`](https://github.com/SeldonIO/seldon-core/pull/6183)
- Remove faulty link [`#6168`](https://github.com/SeldonIO/seldon-core/pull/6168)
- Add Managed Kafka page to latest docs [`#6166`](https://github.com/SeldonIO/seldon-core/pull/6166)
- feat(envoy): fixing a test [`#6163`](https://github.com/SeldonIO/seldon-core/pull/6163)
- Mark kafka as recommended [`#6165`](https://github.com/SeldonIO/seldon-core/pull/6165)
- fix port to 9004 in seldon cli deps [`#6164`](https://github.com/SeldonIO/seldon-core/pull/6164)
- refactor(envoy): add clusters before updating routes (2) [`#6145`](https://github.com/SeldonIO/seldon-core/pull/6145)
- fix(ansible): Upgrade deps in ansible install [`#6146`](https://github.com/SeldonIO/seldon-core/pull/6146)
- feat(k6): add scenario with multiple stages ramping up/down RPS [`#6031`](https://github.com/SeldonIO/seldon-core/pull/6031)
- fix(docs): Docs on upgrading from 2.7 - 2.8 [`#6143`](https://github.com/SeldonIO/seldon-core/pull/6143)
- fix: Add timeout to contexts in client calls [`#6125`](https://github.com/SeldonIO/seldon-core/pull/6125)
- Format spaces in install docs [`#6140`](https://github.com/SeldonIO/seldon-core/pull/6140)
- fix(docs): add a table for core 2 dependencies in docs [`#6139`](https://github.com/SeldonIO/seldon-core/pull/6139)
- feat(scheduler): account for multiple instances of a model per server when scheduling [`#6054`](https://github.com/SeldonIO/seldon-core/pull/6054)
- Bump grafana/grafana from 11.3.1 to 11.4.0 in /scheduler [`#6133`](https://github.com/SeldonIO/seldon-core/pull/6133)
- Bump envoyproxy/envoy from v1.32.1 to v1.32.2 in /scheduler [`#6134`](https://github.com/SeldonIO/seldon-core/pull/6134)
- Bump google.golang.org/grpc from 1.68.0 to 1.68.1 in /hodometer [`#6136`](https://github.com/SeldonIO/seldon-core/pull/6136)
- fix(docs): first draft of the securing endpoints [`#5991`](https://github.com/SeldonIO/seldon-core/pull/5991)
- refactor(envoy): moving envoy/resources headers to util [`#6129`](https://github.com/SeldonIO/seldon-core/pull/6129)
- fix(cli): Kafka inspect output formatting [`#6130`](https://github.com/SeldonIO/seldon-core/pull/6130)
- feat(docs): improve HPA documentation [`#6091`](https://github.com/SeldonIO/seldon-core/pull/6091)
- refactor(envoy): refactoring and optimising the components that build envoy config [`#6119`](https://github.com/SeldonIO/seldon-core/pull/6119)
- Re-generate license info [`#6128`](https://github.com/SeldonIO/seldon-core/pull/6128)
- change default k6 image in kustomize [`#6126`](https://github.com/SeldonIO/seldon-core/pull/6126)
- fix(operator): regenerate CRDs [`#6124`](https://github.com/SeldonIO/seldon-core/pull/6124)
- Bump grafana/grafana from 11.3.0 to 11.3.1 in /scheduler [`#6105`](https://github.com/SeldonIO/seldon-core/pull/6105)
- feat(envoy): add an envoy config snapshot test [`#6121`](https://github.com/SeldonIO/seldon-core/pull/6121)
- fix(envoy): use ADS in dynamic config config [`#6120`](https://github.com/SeldonIO/seldon-core/pull/6120)
- fix(metrics): Fix model label metric in case of experiment [`#6118`](https://github.com/SeldonIO/seldon-core/pull/6118)
- fix(cli): Add error topic to pipeline inspect [`#6117`](https://github.com/SeldonIO/seldon-core/pull/6117)
- Re-generate license info [`#6116`](https://github.com/SeldonIO/seldon-core/pull/6116)
- feat(cli): cli as k8s deployment for debugging [`#6090`](https://github.com/SeldonIO/seldon-core/pull/6090)
- feat: Expose pvc retention policy via helm [`#6056`](https://github.com/SeldonIO/seldon-core/pull/6056)
- feat(envoy): switch to ADS [`#6082`](https://github.com/SeldonIO/seldon-core/pull/6082)
- Re-generate license info [`#6089`](https://github.com/SeldonIO/seldon-core/pull/6089)
- fix(cli): Fix kafka topic assignment for cli [`#6085`](https://github.com/SeldonIO/seldon-core/pull/6085)
- Re-generate license info [`#6083`](https://github.com/SeldonIO/seldon-core/pull/6083)
- fix: Model gateway silently ignores errors [`#6014`](https://github.com/SeldonIO/seldon-core/pull/6014)
- Re-generate license info [`#6080`](https://github.com/SeldonIO/seldon-core/pull/6080)
- Re-generate license info [`#6076`](https://github.com/SeldonIO/seldon-core/pull/6076)
- Bump github.com/go-playground/validator/v10 in /scheduler [`#6067`](https://github.com/SeldonIO/seldon-core/pull/6067)
- Bump github.com/tidwall/gjson from 1.17.1 to 1.18.0 in /operator [`#6060`](https://github.com/SeldonIO/seldon-core/pull/6060)
- Bump sigs.k8s.io/controller-runtime from 0.17.4 to 0.19.1 in /operator [`#6059`](https://github.com/SeldonIO/seldon-core/pull/6059)
- Bump ubi9/ubi-micro from 9.4-15 to 9.5 in /operator [`#6058`](https://github.com/SeldonIO/seldon-core/pull/6058)
- Bump rclone/rclone from 1.68.1 to 1.68.2 in /scheduler [`#6062`](https://github.com/SeldonIO/seldon-core/pull/6062)
- Bump ubi9/ubi-minimal from 9.4-1227.1726694542 to 9.5 in /scheduler [`#6063`](https://github.com/SeldonIO/seldon-core/pull/6063)
- Bump ubi9/ubi-micro from 9.4-15 to 9.5 in /scheduler [`#6064`](https://github.com/SeldonIO/seldon-core/pull/6064)
- Bump ubi9/ubi-micro from 9.4-15 to 9.5 in /hodometer [`#6065`](https://github.com/SeldonIO/seldon-core/pull/6065)
- Bump github.com/envoyproxy/go-control-plane in /scheduler [`#6068`](https://github.com/SeldonIO/seldon-core/pull/6068)
- Bump github.com/rs/xid from 1.5.0 to 1.6.0 in /scheduler [`#6069`](https://github.com/SeldonIO/seldon-core/pull/6069)
- fix(k6): use seldon-mesh svc for envoy k6 tests [`#6070`](https://github.com/SeldonIO/seldon-core/pull/6070)
- fix mismatched dependencies [`#6057`](https://github.com/SeldonIO/seldon-core/pull/6057)
- fixing the envoy dashboard [`#6055`](https://github.com/SeldonIO/seldon-core/pull/6055)
- Re-generate license info [`#6052`](https://github.com/SeldonIO/seldon-core/pull/6052)
- Bump github.com/onsi/gomega from 1.34.0 to 1.35.1 in /scheduler [`#6025`](https://github.com/SeldonIO/seldon-core/pull/6025)
- Bump grafana/grafana from 11.2.0 to 11.3.0 in /scheduler [`#6000`](https://github.com/SeldonIO/seldon-core/pull/6000)
- Bump github.com/onsi/gomega from 1.33.1 to 1.35.1 in /operator [`#6026`](https://github.com/SeldonIO/seldon-core/pull/6026)
- Bump envoyproxy/envoy from v1.31.2 to v1.32.1 in /scheduler [`#6027`](https://github.com/SeldonIO/seldon-core/pull/6027)
- fix(deps): Bump google.golang.org/grpc from 1.65.0 to 1.68.0 in /components/tls [`#6039`](https://github.com/SeldonIO/seldon-core/pull/6039)
- Bump go.opentelemetry.io/contrib/instrumentation/net/http/otelhttp [`#6041`](https://github.com/SeldonIO/seldon-core/pull/6041)
- fix(deps): Bump google.golang.org/grpc from 1.65.0 to 1.68.0 in /apis/go [`#6042`](https://github.com/SeldonIO/seldon-core/pull/6042)
- fix(deps): Bump google.golang.org/grpc from 1.65.0 to 1.68.0 in /operator [`#6043`](https://github.com/SeldonIO/seldon-core/pull/6043)
- fix(deps): Bump google.golang.org/grpc from 1.66.0 to 1.68.0 in /hodometer [`#6037`](https://github.com/SeldonIO/seldon-core/pull/6037)
- fix(docs) Fixed the rendering issues [`#6015`](https://github.com/SeldonIO/seldon-core/pull/6015)
- allow model versions to increase [`#6038`](https://github.com/SeldonIO/seldon-core/pull/6038)
- fix: Upgrade go 1.22 [`#5990`](https://github.com/SeldonIO/seldon-core/pull/5990)
- Update Changelog [`#6035`](https://github.com/SeldonIO/seldon-core/pull/6035)
- Generating changelog for v2.9.0 [`fe57037`](https://github.com/SeldonIO/seldon-core/commit/fe57037361cc1b70c9cd3c43280a90d95dee9f40)
- Generating changelog for v2.9.0-rc2 [`f5e47ed`](https://github.com/SeldonIO/seldon-core/commit/f5e47ed43b7b4bcfbca92463053ae46edded6747)
- Setting version for helm charts [`1ccc180`](https://github.com/SeldonIO/seldon-core/commit/1ccc1802101325fdb3ac6238fcdffb07bf236613)
- Setting version for helm charts [`4865356`](https://github.com/SeldonIO/seldon-core/commit/4865356fbe6e0db68b82d4e28bc32b7f00a36627)
- Setting version for yaml manifests [`456036a`](https://github.com/SeldonIO/seldon-core/commit/456036ace930eca52e46f9d30d8246e6fc728d1c)
