...
function update_images {
    ...
    sed -i 's#\(^.*\)\(- name: RELATED_IMAGE_SKLEARNSERVER$\)#\1\2\n\1  value: '${SKLEARNSERVER_IMAGE}:${VERSION}'#' ${CSV_FOLDER}/seldon-operator.clusterserviceversion.yaml
    sed -i 's#\(^.*\)\(- name: RELATED_IMAGE_XGBOOSTSERVER$\)#\1\2\n\1  value: '${XGBOOSTSERVER_IMAGE}:${VERSION}'#' ${CSV_FOLDER}/seldon-operator.clusterserviceversion.yaml
    sed -i 's#\(^.*\)\(- name: RELATED_IMAGE_MLFLOWSERVER$\)#\1\2\n\1  value: '${MLFLOWSERVER_IMAGE}:${VERSION}'#' ${CSV_FOLDER}/seldon-operator.clusterserviceversion.yaml
    sed -i 's#\(^.*\)\(- name: RELATED_IMAGE_TFPROXY$\)#\1\2\n\1  value: '${TFPROXY_IMAGE}:${VERSION}'#' ${CSV_FOLDER}/seldon-operator.clusterserviceversion.yaml
