...
PKG_BASE=github.com/seldonio/seldon-core/operator
...
for d in apis/machinelearning/*/ ; do

     echo "$d"
     # run the generators
     #"${BINDIR}/deepcopy-gen" -v 9 -i ./api/{$d}/ -O zz_generated_new.deepcopy --go-header-file hack/boilerplate.go.txt

     # "${BINDIR}/client-gen" -v 9 --input-base ${PKG_BASE}/apis --clientset-name versioned -i ./apis/machinelearning/${d}/ --input machinelearning/${d} --output-package ${PKG_BASE}/client/clientset --go-header-file hack/boilerplate.go.txt -o ${FAKE_GOPATH}/src

     # "${BINDIR}/lister-gen" -v 5 -i ${PKG_BASE}/apis/machinelearning/${d} --output-package ${PKG_BASE}/client/listers --go-header-file hack/boilerplate.go.txt -o ${FAKE_GOPATH}/src
