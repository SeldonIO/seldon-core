...
class TestLocalOperators(object):
    def test_namespace_operator(self, namespace):
        retry_run(
            f"helm install seldon ../../helm-charts/seldon-core-operator --namespace {namespace} --set ambassador.enabled=true --set istio.enabled=true --set istio.gateway=istio-system/seldon-gateway --set certManager.enabled=false --set crd.create=false --set singleNamespace=true"
        )
    ...
    def test_labelled_operator(self, namespace):
        retry_run(
            f"helm install seldon ../../helm-charts/seldon-core-operator --namespace {namespace} --set ambassador.enabled=true --set istio.enabled=true --set istio.gateway=istio-system/seldon-gateway --set certManager.enabled=false --set crd.create=false --set controllerId=seldon-id1"
        )
        retry_run(
