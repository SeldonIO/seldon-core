...
class TestLocalOperators(object):
    ...
    def test_namespace_operator(self):
        ...
        retry_run(f"helm install seldon ../../helm-charts/seldon-core-operator --namespace {namespace} --set istio.enabled=true --set istio.gateway=seldon-gateway --set certManager.enabled=false --set crd.create=false --set singleNamespace=true")
    ...
    def test_labelled_operator(self):
        ...
        retry_run(f"kubectl create namespace {namespace}")
        retry_run(f"helm install seldon ../../helm-charts/seldon-core-operator --namespace {namespace} --set istio.enabled=true --set istio.gateway=seldon-gateway --set certManager.enabled=false --set crd.create=false --set controllerId=seldon-id1")
        ...
        run(f"kubectl delete -f ../resources/model_controller_id.yaml -n default", shell=True)
        run(f"kubectl delete namespace {namespace}", shell=True)