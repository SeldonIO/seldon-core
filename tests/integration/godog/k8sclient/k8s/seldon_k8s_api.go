...
func (k *SeldonK8sAPI) Create(spec string) error {
	resource, err := getResource(spec)
	if err != nil {
		return err
	}
...
func (k *SeldonK8sAPI) waitForModelState(state, model, timeout string) error {
	// TODO we should use watch to reduce latency... might require using client-gen to generate client for our CRs

	return wait.PollUntilContextTimeout(context.TODO(), 2*time.Second, 5*time.Minute, true, func(ctx context.Context) (bool, error) {
		var modelCR *v1alpha1.Model
		err := k.k8sClient.Get((ctx, types.NamespacedName{
		...
		// TODO how to check for "Ready"???
		// Check your ready condition
		return modelCR.Status.IsReady() == "Ready", nil
	})
}
