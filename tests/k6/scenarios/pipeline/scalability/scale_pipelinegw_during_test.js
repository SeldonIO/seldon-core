...
// runScalePipelineGwOpTimes how many times to scale up pipeline-gw during test, it will be evenly distributed across
// the progress of the test
const runScalePipelineGwOpTimes = 3
...
// replicaCountPipelineGw how many replicas for pipeline-gw
let replicaCountPipelineGw = replicaPipeLineGw;


export default function (config) {
    if (exec.vu.idInTest === vuIDToRunScaleOps) {
        if (exec.scenario.progress >= ((scaledOpRunHistory + 1) / (runScalePipelineGwOpTimes + 1))) {
            replicaCountPipelineGw++;
            console.log("Scaling UP pipeline-gw to " + replicaCountPipelineGw + " replicas at " + Math.round(exec.scenario.progress * 100) + "% progress");

            k8s.init()
            const ctl = connectControlPlaneOps(config)
            const seldonRuneTime = generateSeldonRuntime(replicaModelGw, replicaCountPipelineGw, replicaDataFlowEngine)
            ctl.loadSeldonRuntimeFn(seldonRuneTime.object, false, true)
            scaledOpRunHistory++;
        }
    }
