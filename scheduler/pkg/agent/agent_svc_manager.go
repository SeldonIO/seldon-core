...
func (am *AgentServiceManager) HandleSchedulerSubscription() error {
	...
	if am.agentConfig.runningInK8s {
		logger.Infof("Checking pod has IP published in endpoints")
		...
		logger.Info("Pod has IP published in endpoints")
		//
		//// backoff retry is required, as we've only just changed isStartup to false, it was true before which would have
		//// caused the readiness probe to fail. We retry until kubelet calls the probe again.
		//err = boff.RetryNotify(func() error {
		//	if !am.readinessService.(*readyservice.ReadyService).PassedK8sReadinessCheck() {
		...
		//		// In the case of not ready, will backoff retry indefinitely.
		//		return fmt.Errorf("agentServiceManager is not ready to receive scheduler operations")
		//	}
		//	return nil
		//}, util.GetClientExponentialBackoff(), func(err error, duration time.Duration) {
		//	am.logger.WithError(err).Warn("Readiness not yet passed k8s probe")
		//})
		//if err != nil {
		//	return fmt.Errorf("backoff readiness check failed: %v", err)
		//}
	}
